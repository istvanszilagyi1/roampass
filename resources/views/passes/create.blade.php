...
<div id="payment-modal" class="fixed inset-0 z-[9999] hidden items-center justify-center bg-black/95 backdrop-blur-md opacity-0 transition-opacity duration-300" style="pointer-events: none; margin-top: 50px;">
            ...
            <form id="simulation-form" method="POST" action="{{ route('passes.store') }}" onsubmit="handlePaymentSubmit(event)" class="space-y-4 m-0">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="relative">
                            <input type="password" id="cvc" placeholder="123" class="pay-input no-icon text-center" maxlength="3" inputmode="numeric" pattern="\d*" oninput="this.value = this.value.replace(/[^0-9]/g, '')" required>
                        </div>
                        <div class="error-message" id="cvc-error">Kötelező mező</div>
                    </div>
                </div>
                <div class="error-message text-center pt-1 text-red-400 text-xs" id="general-error">Kérlek tölts ki minden mezőt megfelelően!</div>

                <button type="submit" id="pay-submit-btn"
                        class="w-full bg-emerald-600 hover:bg-emerald-500 text-white py-3.5 rounded-xl font-black text-sm uppercase tracking-widest transition-all mt-4 flex justify-center items-center gap-2 shadow-[0_0_15px_rgba(16,185,129,0.3)] cursor-pointer">
